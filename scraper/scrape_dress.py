...
async def download_images(image_links, save_dir="static/images/downloaded"):
...
def scrape_endpoint():
    ...
    url = data.get("url")
    result = asyncio.run(scrape_dress(url))

    os.makedirs("static/data", exist_ok=True)

    with open("static/data/dress_details.txt", "w", encoding="utf-8") as f:
        f.write("EDITOR'S NOTES:\n" + result["editors_notes"] + "\n\n")
        f.write("SIZE & FIT:\n")
        for detail in result["size_fit"]:
            f.write("- " + detail + "\n")
        f.write("\nMODEL MEASUREMENTS:\n")
        for measurement in result["model_measurements"]:
            f.write("- " + measurement + "\n")
        f.write("\nDETAILS & CARE:\n")
        for item in result["details_care"]:
            f.write("- " + item + "\n")

    with open("static/data/Size_guide.json", "w", encoding="utf-8") as f:
        json.dump(result["size_guide_popup"], f, ensure_ascii=False, indent=2)

    return jsonify({"message": "Scraping complete and data saved."})
